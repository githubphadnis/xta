...
def test_insights_most_visited_store_and_month_filter():
    TestingSessionLocal = _setup_test_db()
    db = TestingSessionLocal()
    db.add_all(
        [
    ...
    response = client.post(
        "/api/insights/ask",
        data={"question": "Which store did I visit the most?", "month": "2026-02"},
        headers=headers,
    )
    app.dependency_overrides.clear()
    assert response.status_code == 200
    payload = response.json()
    assert payload["chart"]["labels"][0] == "Store A"
    assert payload["chart"]["values"][0] == 2.0
